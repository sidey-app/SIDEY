...
> 현재 공개 버전은 `v1.0.1`(build 12)입니다. 설치 파일과 자동 업데이트 피드는 같은 버전을 가리킵니다.
...
```sh
./scripts/package_macos_release.sh v1.0.1
```
...
SIDEY_RELEASE_NOTES=/path/to/release-notes.md \
  ...
  v1.0.1 \
  build/releases/v1.0.1/SIDEY-macOS-arm64-v1.0.1.zip
```
