...
현재 공개 버전은 `v1.2.0`(build 28)입니다. 직배포판의 실행 표시 이름은 `SIDEY-DIRECT`이며, 설치 파일 안의 `SIDEY.app`을 기존 앱과 같은 위치에 옮기면 됩니다. App Store판은 `SIDEY`로 표시됩니다.
...
### macOS · 2026년 9월 12일 · v1.2.0

- 아기 수달·아기 돼지·나무와 새로운 투척물의 미리보기를 추가했습니다.
- 캐릭터와 애착 물건을 나란히 확인하고 각각의 구매 상태를 볼 수 있도록 상점을 개편했습니다.
- 미리보기에서 상대 캐릭터를 누르면 애착 물건을 던지며 효과음을 함께 들을 수 있습니다.
- 나무를 우클릭하면 멈추고 다시 우클릭하면 걸으며, 던지기 활성화는 더블 우클릭으로 바뀌었습니다.
- 직배포판 실행 이름을 `SIDEY-DIRECT`로 구분하고 상점의 여백과 가격 상태 표시를 개선했습니다.
