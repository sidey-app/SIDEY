...
public static class WindowsOverlayRegionLayout
{
        ...
        OverlayRegionPreference preference)
    {
        ...
        int preferredDepth = Round(PreferredDepthDip * dpi / 96d);
        int depth = Math.Max(1, Math.Min(preferredDepth, workAreaPixels.Height / 3));
        int maximumRenderDepth = Math.Max(1, Round(MaximumRenderDepthDip * dpi / 96d));
        int maximumPadding = Math.Max(0, Round(MaximumTangentPaddingDip * dpi / 96d));
        if (preference.Edge is OverlayEdge.Bottom or OverlayEdge.Top)
        {
            int length = Math.Max(1, Round(workAreaPixels.Width * preference.Span.Fraction()));
            ...
            int renderDepth = Math.Min(maximumRenderDepth, workAreaPixels.Height);
        ...
        int verticalLength = Math.Max(1, Round(workAreaPixels.Height * preference.Span.Fraction()));
        ...
        int horizontalRenderDepth = Math.Min(maximumRenderDepth, workAreaPixels.Width);
