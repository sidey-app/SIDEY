...
- 루트 런처, `SIDEY.Host.exe`, `SIDEY.Host.dll`, `Sidey.*.dll`은 SIDEY 인증서로 서명합니다. 공급자 런타임 파일의 기존 서명은 유지합니다.
- 사용자 설정, 자격 증명과 메시지 상태는 Program Files가 아니라 Windows 사용자 프로필 또는 Credential Manager에 유지합니다.
