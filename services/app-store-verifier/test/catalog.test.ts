...
test("legacy and solo product IDs remain accepted while typos fail closed", () => {
  assert.equal(Object.keys(entitlementByProduct).length, 33);
  ...
  for (const id of ["character_monkey_solo_3", "character_monkey_solo_2", "throwable_clam_2", "throwable_pork_2"])
    assert.equal(isSideyProductID(id), true);
  assert.equal(isSideyProductID("character_monkey_solo_4"), false);
