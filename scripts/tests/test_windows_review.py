...
class WindowsReviewTests(unittest.TestCase):
    def test_requires_current_main_push_and_actual_smoke(self):
        metadata = dict(head_sha='current', head_branch='main', event='push', status='completed',
                        conclusion='success', path='.github/workflows/validate-change.yml')
        jobs = [dict(name='Windows validation', conclusion='success',
        ...
        for invalid in [[], [dict(name='Windows validation', conclusion='success', steps=[])],
                        [dict(name='Windows validation', conclusion='skipped')],
                        [dict(name='Windows validation', conclusion='success', steps=[dict(name='Run Windows app smoke', conclusion='skipped')])]]:
