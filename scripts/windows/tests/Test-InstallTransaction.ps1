...
try {
    ...
    $initialState = Get-Content -LiteralPath ($install + '.sidey-transaction.json') -Raw | ConvertFrom-Json
    ...
    $activeState = Get-Content -LiteralPath ($install + '.sidey-transaction.json') -Raw | ConvertFrom-Json
    ...
    Invoke-Transaction Prepare
    $nextState = Get-Content -LiteralPath ($install + '.sidey-transaction.json') -Raw | ConvertFrom-Json
