...
if ($manifest.Count -ne 15 -or @($manifest.id | Select-Object -Unique).Count -ne 15) { throw 'Expected fifteen unique approved impact sounds.' }
...
if (@(Get-ChildItem -LiteralPath $impactAudioDirectory -Filter '*.wav' -File -Recurse).Count -ne 15) { throw 'Unexpected impact audio files.' }
Write-Output 'Verified fifteen approved impact WAV files.'
