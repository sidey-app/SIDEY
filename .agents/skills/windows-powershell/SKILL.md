...
- For compiled prerequisite detection, error mapping, or legacy private-runtime cleanup, run `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File scripts/windows/tests/Test-PrerequisiteInstaller.ps1`. Without `-HelperPath`, it builds and exercises the helper in an isolated temporary directory; it reads local package and registry state but does not provision prerequisites.
- For an existing publish tree, run `scripts/windows/tests/Test-FrameworkDependentPublish.ps1 -PublishDirectory <path>` and `scripts/windows/tests/Test-FrameworkPublishGuard.ps1 -PublishDirectory <path>`. These checks also require current restored project assets.
- For startup behavior, run `scripts/windows/Install-WindowsPrerequisites.ps1 -Version <version>` before `scripts/windows/tests/Test-PublishedApplication.ps1 -PublishDirectory <path>` only when network downloads, prerequisite installation, and starting and stopping actual SIDEY processes are in scope. The prerequisite installer registers Windows App Runtime for the actual desktop user and has no machine-wide provisioning switch; do not add or pass `-ProvisionAllUsers`.
...
- Treat `scripts/windows/tests/Test-WindowsBuild.ps1` as an extended machine-level route: it restores, builds, tests, publishes, may install missing prerequisites, and starts the application. Do not use it as the default local check.
