...
- For a publish tree, inspect the current distribution model and use the narrow publish-layout checks wired into `scripts/windows/tests/Test-WindowsBuild.ps1`. Do not assume the application is framework-dependent or that a shared runtime must be installed.
- For startup behavior, run `scripts/windows/tests/Test-PublishedApplication.ps1 -PublishDirectory <path>` only when starting and stopping actual SIDEY processes is in scope. Inspect the current publish contract before provisioning or changing any machine prerequisite.
- For installer or helper-generation changes, run `scripts/windows/New-WindowsInstaller.ps1` with an explicit publish and output directory. It requires NSIS and exercises the maintained payload checks, helper reproducibility, install transactions, and the ban on PowerShell in the shipped installer runtime.
- Treat `scripts/windows/tests/Test-WindowsBuild.ps1` as an extended machine-level route: it restores, builds, tests, publishes, follows the current distribution checks, and starts the application. Depending on the current distribution model, it may use the network or change machine prerequisites. Do not use it as the default local check.
- Treat update-design, installer-UI, and release verification scripts as manual or authorized routes because they can start UI, use the network, or change machine state.
