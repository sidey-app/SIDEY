...
- For startup behavior, run `scripts/windows/Install-WindowsPrerequisites.ps1 -Version <version>` before `scripts/windows/tests/Test-PublishedApplication.ps1 -PublishDirectory <path>` only when network downloads, prerequisite installation, and starting and stopping actual SIDEY processes are in scope.
