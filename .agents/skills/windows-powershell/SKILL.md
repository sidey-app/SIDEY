...
- For startup behavior, run `scripts/windows/Install-WindowsPrerequisites.ps1 -Version <version>` before `scripts/windows/Test-PublishedApplication.ps1 -PublishDirectory <path>` only when network downloads, prerequisite installation, and starting and stopping actual SIDEY processes are in scope. Do not pass `-ProvisionAllUsers` unless machine-wide Windows App Runtime provisioning is explicitly authorized.
