...
- Finish squash-merges the exact checked PR head, verifies that the resulting commit tree matches it and fast-forwards the primary main worktree. A blocked primary update is incomplete integration. Platform tasks remain `main-updated` until app verification; for macOS use `open --task <task-id>` on latest main to record completion. Never merge unrelated PRs or unfinished branches to make main “latest.”
