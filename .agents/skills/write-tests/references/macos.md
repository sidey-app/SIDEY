...
Prefer a focused XCTest or Python test while iterating. `./scripts/macos/tests/test_native.sh` is the canonical broad route and runs asset verification, Python tests, both Direct and App Store XCTest schemes, and recording tests. Do not substitute application launch, signing, StoreKit, Keychain or network-backed checks without matching authorization.
