...
Run the nearest affected test first, then use the non-installing solution sequence in `windows/AGENTS.md` and any affected artifact check. Do not use `scripts/windows/Test-WindowsBuild.ps1` by default; it publishes, may install prerequisites and starts executables.
