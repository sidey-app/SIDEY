...
final class CharacterFeedbackDebugRoom: NSWindowController, NSWindowDelegate {
    ...
    init() {
        ...
        window.title = "SIDEY · 기절·효과음 테스트 방"
        window.isReleasedWhenClosed = false
        window.delegate = self
        let content = window.contentView!
        addLabel("로컬 테스트 방 · 나 + 콩이", frame: CGRect(x: 24, y: 471, width: 500, height: 25), size: 20, to: content)
        ...
        addLabel("친구 캐릭터", frame: CGRect(x: 315, y: 431, width: 90, height: 22), to: content)
        ...
        world = PixelWorldScene(size: spriteView.bounds.size, renderingConfiguration: .storePreview(
            fixedTrackFractions: [actorID: 0.27, friendID: 0.73]))
        ...
        addLabel("콩이를 클릭하거나 Space를 누르세요. 0.5초 간격 · 10초 내 10회 피격 → 6초 기절 · 보호 시간 없음",
    ...
    private func applyMembers() {
        world.apply(roomID: roomID, members: [
            PixelWorldMember(id: actorID, nickname: "토리", characterID: "pixel_hamster", presence: .online, isTyping: false, isCurrentUser: true),
            PixelWorldMember(id: friendID, nickname: "콩이", characterID: PixelCharacterCatalog.all[characterPicker.indexOfSelectedItem].id,
    ...
    private func throwObject(from actor: UUID, to target: UUID) {
        ...
        guard !world.stunState.isStunned(actor, at: now), cooldown.accept(actorUserID: actor, uptime: now) else { return }
        world.playLocalPreviewThrow(CharacterThrowEvent(id: UUID(), roomID: roomID, actorUserID: actor,
            targetUserID: target, sourceCharacterID: "pixel_hamster", throwableID: choices[throwablePicker.indexOfSelectedItem].1))
    }
    ...
    @objc private func changeCharacter() { applyMembers() }
