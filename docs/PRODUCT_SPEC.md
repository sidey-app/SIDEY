...
닉네임 상태 점은 고정 x 좌표를 사용하지 않는다. 실제 닉네임 label frame의 왼쪽에서 점 반지름과 5pt 간격만큼 떨어진 위치를 계산해 최대 8자 이름과 `· 나` 표식에서도 겹치지 않게 한다.
...
- 내 캐릭터 클릭 또는 메뉴바 `메시지 작성`으로 열고 즉시 포커스한다.
...
`OverlayWindowGroup`은 composer 표시 상태와 내 캐릭터 클릭 패널을 소유한다. `AppModel`에는 화면 좌표나 애니메이션 frame을 저장하지 않는다.
...
- 이동: 20개 합성 노드가 3,000 tick 동안 1차원 track과 발 기준선을 유지하고 finite 좌표, 입력창·상호 회피, 혼잡 시 안전한 겹침
- 상태: 온라인·자리 비움 doze·오프라인 curled sleep·재연결·타이핑, 내 캐릭터 항상 표시, 오프라인 숨김, 방 전환 UUID diff, 최대 8자 닉네임과 상태 점 5pt 간격
...
- 창: 월드 항상 위·클릭 통과, 내 캐릭터 52×52 hotspot만 포인터 수신, composer 초기 숨김·열기·Esc·마지막 전송 뒤 10초 자동 닫힘·타이머 갱신·실패 복구, 기록 일반 창
