...
- macOS 그룹 설정에서 모든 멤버는 기본 접힘 상태인 각 그룹을 펼쳐 캐릭터·닉네임·`나` 표시를 볼 수 있다. 방장은 닉네임 바로 왼쪽의 금색 `crown.fill`로 표시하며 오버레이 이름표에는 왕관을 넣지 않는다.
- 현재 `owner_id`인 방장만 그룹 이름 변경, 본인을 제외한 멤버 추방, 그룹 삭제 UI를 볼 수 있다. 이름은 앞뒤 공백을 제거한 줄바꿈·탭 없는 1~20자이며, 추방은 대상 닉네임을 표시한 확인을 거친다. 삭제는 멤버와 모든 메시지가 영구 삭제된다는 파괴적 2단계 확인을 거친 hard delete다.
...
macOS 메뉴바의 `캐릭터 이동 모드`를 켜면 현재 사용자 여부와 관계없이 표시 중인 캐릭터를 드래그할 수 있다. 놓은 위치는 activity frame의 접선 범위로 투영하고, 캐릭터는 선택 가장자리 방향으로 중력 낙하해 최대 3회 감쇠 바운스한 뒤 해당 위치의 track에 착지하고 기존 로컬 산책으로 복귀한다. 드래그 위치와 낙하 상태는 클라이언트별 `PixelWorldScene` 일시 상태이며 저장하거나 Realtime로 동기화하지 않는다.
...
- 기본 모드에서는 전체 영역이 `ignoresMouseEvents`로 뒤 애플리케이션에 클릭을 통과시킨다. 메뉴바에서 명시적으로 `캐릭터 이동 모드`를 켠 동안에만 월드 패널이 창 내부 포인터 이벤트를 받는다.
...
기본 모드에서 월드 패널 전체는 `ignoresMouseEvents`로 클릭을 통과시킨다. 내 캐릭터 위의 52×52pt 투명 `NSPanel`만 최대 15Hz·1pt 임계값으로 위치를 따라가며 메시지·리액션 클릭을 받는다. 캐릭터 이동 모드를 켜면 이 hotspot을 숨기고 월드 패널에서 내 캐릭터와 다른 멤버 캐릭터의 드래그를 처리한다. 모드를 끄거나 오버레이를 숨기면 즉시 기본 클릭 통과로 복귀한다. 전역 마우스 좌표와 전역 이벤트 모니터는 사용하지 않으며, 창 내부 드래그 위치도 저장·로그·전송하지 않는다.

오버레이 영역 자체의 자유 이동·크기 조절·잠금과 오버레이 내부의 최근 기록·설정 버튼은 제거한다. 캐릭터 직접 이동은 별도 이동 모드의 로컬 일시 상호작용으로만 제공한다. 최근 기록은 메뉴바에서 여는 일반 레벨의 macOS 창으로 제공한다.
...
- 캐릭터 드래그 대상·오프셋과 낙하·감쇠 바운스 일시 상태
...
캐릭터 이동 모드는 월드 창 안에서 발생한 드래그 위치를 해당 이벤트 처리 중에만 사용한다. 이 창 내부 좌표는 저장·로그·서버 전송하지 않으며 전역 이벤트 모니터나 전역 포인터 조회로 확장하지 않는다.
...
- 상태: 온라인·자리 비움 doze와 주황 `Zzz`·오프라인 curled sleep·재연결·타이핑, 내 캐릭터 항상 표시, 오프라인 숨김, 방 전환 UUID diff, 최대 8자 닉네임·반투명 배경과 상태 점 5pt 간격, 내·다른 캐릭터 로컬 드래그와 4방향 최대 3회 감쇠 바운스 착지, 발 기준 7배·0.8초 리액션과 이벤트 UUID 중복 제거
...
- 창: 월드 항상 위·기본 클릭 통과, 내 캐릭터 52×52 hotspot과 명시적 캐릭터 이동 모드의 포인터 소유권 전환, composer의 선택 모니터 상단 중앙·노치 아래 10pt 배치와 왼쪽 `×`·Esc·외부 클릭 닫기, 단일·더블클릭 분기와 캐릭터별 1초 리액션 쿨타임, composer 초기 숨김·열기·마지막 전송 뒤 5초 자동 닫힘·타이머 갱신·실패 복구, 기록 일반 창
...
- 그룹 설정: 0·1·5명 멤버 목록, 모든 사용자의 펼침, UUID 기반 방장 왕관과 방장 전용 관리 버튼, 이름 변경 저장·취소, 대상 명시 추방 확인, 영구 삭제 2단계 확인, 활성·비활성·마지막 그룹 삭제 fallback
