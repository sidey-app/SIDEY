...
Windows 그룹 페이지는 활성 그룹의 사람 아이콘을 민트색 채움으로, 비활성 그룹은 회색 윤곽으로 표시하며 그룹 전환 상태에 맞춰 갱신한다. 미니 대포 발사대는 네 가장자리에서 상대를 향하도록 회전·반전하고 macOS와 같이 캐릭터 중심에서 진행 방향 6pt·바깥쪽 1pt에 배치한다. 미리보기 발사체·발사대·충돌 효과는 화면 배치 크기를 유지하고 투명도로 표시를 전환해 프레임 사이에 사라지거나 깜빡이지 않도록 한다.
