...
- 트레이 우클릭 메뉴는 설정과 중복되는 `열기` 항목 없이 오버레이, 메시지 작성, 활성 그룹, 조용히 모드, 최근 기록, 상점, 그룹 설정, 로그인 실행, 업데이트 확인, 설정, `종료`를 제공하고 구분선을 제외한 모든 항목에 항목별 아이콘을 표시한다. 트레이 아이콘 왼쪽 클릭·더블클릭과 연결 실패 알림 클릭은 기본 창을 연다. 익명 세션을 사용하는 Windows판에는 로그아웃 메뉴를 두지 않는다.
