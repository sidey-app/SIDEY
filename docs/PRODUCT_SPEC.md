...
Windows 영어·일본어는 단어별 직역보다 현지 사용자에게 자연스러운 앱 문구와 동물 이름, 일상 대화를 사용하되 기능·제한·결제 의미를 바꾸지 않는다. 선택한 언어의 표준 날짜·시간·숫자 형식을 사용하고 시간대는 시스템의 실제 현지 시간대를 유지한다. 상품 가격은 언어와 무관하게 실제 결제 통화인 원화로 표시하며 환율을 추정해 바꾸지 않는다.
