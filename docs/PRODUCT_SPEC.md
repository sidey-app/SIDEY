...
- 문서 버전: 0.8
- 최종 갱신: 2026-09-10
- 상태: macOS `v1.1.0`(build 24) 정식 공개·production 상점 판매 잠금, Windows 네이티브 `v1.2.0` 정식 출시
...
- Windows 기본 CTA는 현재 정식 v1.2.0의 고정 Setup EXE를 직접 가리키며 저장소에서부터 활성 링크로 제공한다. Pages Actions는 버전에 맞는 정식 Release의 단일 Windows 설치 자산과 고정 URL을 확인한 경우에만 배포를 계속한다.
