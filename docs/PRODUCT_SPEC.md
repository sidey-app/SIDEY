...
- 탐색 푸터의 연결 상태는 하나의 레이아웃 자리에서 펼친 카드와 축소 점을 교체해 전환 중 두 점이나 잘린 카드 배경이 나타나지 않게 한다. 연결 상태는 초록 점과 `연결됨`, 단절 상태는 빨간 점과 `연결 안 됨`으로 표시한다. 앱 시작 뒤 최초 연결 전에는 Windows 시스템 알림을 15초 유예하고 그 안에 연결되면 예약을 취소한다. 15초가 지나도 연결되지 않거나 한 번 정상 연결된 뒤 다시 중단되면 장애당 한 번 알리고, 짧은 연결 반복에 따른 재알림은 15분 동안 억제한다.
