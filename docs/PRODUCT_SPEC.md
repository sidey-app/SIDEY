...
Windows UI 문구는 `ko-KR`·`en-US`·`ja-JP` 카탈로그를 사용하며 시스템 UI 언어를 감지한다. `SIDEY_LANGUAGE`로 로컬 검증 언어를 지정할 수 있고 다른 언어는 한국어로 fallback한다. 런처·표준 컨트롤·미리보기 닉네임과 예시 메시지에도 같은 언어를 적용한다. 언어별 키와 포맷 인자 일치를 자동 검사하며, 상점 미리보기에 별도의 더블클릭 조작 안내 문장은 표시하지 않는다. macOS 구현과 Windows 설치 약관의 한국어 원문은 이 변경으로 수정하지 않는다.
