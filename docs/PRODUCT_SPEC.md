...
- 프로필의 닉네임·설명과 입력창은 충분한 폭에서는 좌우로, 좁은 폭에서는 닉네임·설명·입력창 순서로 세로 배치한다. 캐릭터·말풍선·투척물 선택 그리드는 좁은 폭에서 2열 여러 행을 기본으로 가용 폭에 맞춰 열 수를 조정한다. 항목과 선택 표시는 잘리지 않아야 하며, 레이아웃 전환은 Windows 애니메이션 설정이 켜져 있을 때만 부드럽게 표시한다.
