...
말풍선 색 계약은 핑크 토끼 배경 `#F7A9B8`·글자 `#1C1F29`(8.86:1), 버터 병아리 `#FFE38A`·`#1C1F29`(13.00:1), 별밤 고양이 `#403A78`·`#FFF7E8`(9.42:1)이다. 16×16 좌상단 장식을 메시지와 타이핑 말풍선 모두에 표시한다. 장식 여백만 추가하고 기존 문자열 측정·최대 폭·꼬리·두 개 스택·접선 보정·충돌 규칙은 보존한다.
...
- 타일을 누르면 큰 미리보기와 `구매 / 장착 / 장착 해제` 중 현재 상태에 맞는 한 action을 제공하는 상세 sheet를 연다. 상품별 로딩·오류는 다른 타일의 상태를 막지 않으며 키보드 탐색, 명확한 focus ring, VoiceOver 이름·가격·소유·장착 상태와 action 레이블을 제공한다.
...
- direct development와 App Store의 캐릭터 상품 카드를 선택하면 폭 600pt·최소 높이 580pt 상세 시트를 열고 상단에 540×280pt SpriteKit 미리보기 무대를 하나만 배치한다. 상품 타일 자체는 정적 이미지를 유지한다. 무대 하단에는 라이트·다크 appearance에 대응하는 중립색 4px 픽셀 플랫폼을 24pt 높이로 그리며 신규 PNG를 추가하지 않는다.
...
- 캐릭터 상품은 해당 캐릭터 한 명을 초록 상태 점과 닉네임 `모카`로 표시해 걷기와 idle을 반복한다. 말풍선 상품은 햄스터 `모카`가 걸으면서 해당 테마로 `저메추좀 해줘`를 계속 표시하고 실제 배경·글자·장식·꼬리·가장자리 위치 보정을 사용한다.
...
- 상세에는 반복 애니메이션 재생·일시 정지 버튼과 VoiceOver label을 제공한다. macOS Reduce Motion이 켜져 있으면 상세 시트를 열 때 기본 일시 정지하고 사용자가 직접 재생할 수 있다. 상세 닫기·일시 정지·해체 시 반복 throw 작업을 즉시 취소하고 SKView에서 scene을 제거한다. 동시에 활성화되는 상세 SKView는 최대 하나다.
- 활성 상품 타일은 pointer hover 시 0.14초 동안 2pt 올라가고 배경 명도가 증가하며 accent 테두리를 표시한다. 픽셀 이미지 선명도를 해치는 카드 전체 scale은 사용하지 않는다. 키보드 focus도 accent 테두리로 식별한다. production 잠금 카드는 hover·focus action·상세·animation을 만들지 않는다.
...
- 상점: 760pt 6열과 좁은 폭 5열·4열, 세 탭, 한 줄 타일 상태, 상세 sheet의 구매·장착·해제, 상품별 독립 loading/error, 키보드 탐색·focus·VoiceOver, 860×640·1000×760 라이트·다크 렌더를 검증한다. 상세의 540×280 무대, 48pt 캐릭터·4px 플랫폼, 3배 pulse가 잘리지 않아야 하며 Reduce Motion 기본 일시 정지와 수동 재생을 확인한다. 시나리오별 멤버·닉네임·초록 상태·말풍선 본문/스타일, 캐릭터·말풍선 이동과 투척물 캐릭터 고정, 0.35초 첫 투척·정확한 1초 간격·좌우 교대·상품 ID, 일반 투척물과 대포의 throw/projectile/impact/hit 순서를 검사한다. 상세 시트를 반복해서 열고 닫은 뒤 throw 작업과 SpriteKit scene이 남거나 메모리가 지속 증가하지 않아야 한다. production은 action 호출과 animation Task·SKView가 0개이고 검정 68%·픽셀 자물쇠·잠금 안내만 접근성에 노출되어야 한다. App Store 타깃은 기존 캐릭터 4종 동적 상세와 전체 빌드를 회귀 검증한다.
