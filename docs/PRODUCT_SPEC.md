...
- 기준 가격: 모든 상품 1,100원, 예외는 별빛 우파루파·말풍선 3종·두쫀쿠·왁뿌볼 2,200원과 미니 대포 3,300원이다. 기존 거래 금액은 보존하고 App Store는 Apple의 현지 가격만 표시한다.
- macOS 두 배포판에 승인 캐릭터 5종과 투척물 4종을 추가한다. 삑삑 오리를 재사용하여 캐릭터 17종·기본 포함 투척물 19종·유료 상품 33개가 된다. 물건 소유권은 별도이며 기본 투척물은 모두 말랑공이다. Windows는 기존 콘텐츠를 유지하고 가격과 나무 공유를 반영한다.
- 나무 정지는 본인 프로필 RPC로 원하는 bool과 expected revision을 저장한다. revision 0에서만 기존 기기 설정을 이관하며, 이후 방 이동·재접속·다중 기기에서 서버값을 따른다. 늦은 snapshot과 RPC 응답은 사용자별 최신 revision을 되돌리지 못한다. 저장 실패 시 기존 상태를 유지하고 저장 중 중복 요청을 막는다. 서버 필드가 아직 없으면 본인의 기존 기기 설정을 유지하며, revision 0의 이관 대기·실패 중에도 기존 정지를 유지한다. revision 1 이상부터 서버값이 우선한다. 상점 미리보기와 더블 우클릭 동작은 유지한다.
- 신규 5종의 노출 발목은 몸 밑선에 연결하고 발바닥은 2픽셀 폭으로 맞춘다. 기존 얼굴·의상·색·기준선과 웅크린 자세는 유지하며 승인 원본에서 파생한 90프레임을 재현 검증한다. 공용·웹·두 macOS 번들의 결과와 출처를 함께 검사한다.
- 두 macOS 앱 모두 SIDEY.app/SIDEY로 빌드하되 모듈·scheme·bundle ID는 유지한다. App Store 전용 테스트 타깃으로 기절·자산·가격 경로를 검사하고 배포판별 빌드 디렉터리를 분리한다.
- 기절은 두 macOS 배포판에서 10초 내 중복 없는 10회 충돌 → 6초다. 추가 피격은 연장·재누적하지 않는다. 이동·확대·새 투척은 막고 채팅·발사된 물체는 유지한다. 방 전환·연결 단절 시 임시 상태를 정리하고 회복 후 나무 정지 상태를 유지한다.
- 공용 가격 → 플랫폼 카탈로그 → 신규 공용 콘텐츠 → macOS 연결 → macOS 지원 선언 순서로 검토·통합한다. 각 플랫폼 번들은 검토된 원본 커밋과 해시를 기록한다. 웹 구매 안내는 실제 출시 상태를 따른다.
- 이번에는 버전 인상·앱 공개 릴리스·심사 제출·운영 DB 적용을 하지 않는다. 아래의 과거 가격·콘텐츠 수·직배포 전용 기절·로컬 나무 정지 설명은 이 절의 확정 변경으로 대체하며, 실제 제공은 백엔드 배포와 클라이언트 업데이트 이후다.

서버 상점 카탈로그가 앱보다 적은 정상 부분 응답은 지원한다. 응답된 상품의 메타데이터 검증을 유지하고, 미등록 상품만 구매 불가로 표시한다. 누락만으로 소유권·장착을 회수하지 않으며, 중복 ID·알려진 상품 메타데이터 불일치는 오류로 처리한다.
...
- macOS 설정과 메뉴바에서 공통 catalog의 유료 33종(캐릭터 12·말풍선 3·투척물 18)을 제공하되 direct production 구매는 잠근다. 격리된 Sidey-dev만 Google identity를 연결한 뒤 PortOne 테스트 결제를 시작한다.
- direct판 카탈로그는 승인된 말풍선 3종과 투척물 18종을 포함하되 production의 `.comingSoon`과 서버 `sales_enabled=false`는 유지한다. 결제·지급·자동 장착·해제·환불 회수는 Sidey-dev와 staging에서만 검증한다.
...
- App Store판은 공통 catalog의 33종을 StoreKit 2 비소모성 상품으로 연결하며 서버가 Apple JWS와 App Store Server API 결과를 검증한 뒤에만 entitlement를 지급한다. 로컬 StoreKit 기준 가격은 확정된 1,100·2,200·3,300원이며, 실제 표시는 Apple이 반환하는 현지화 가격을 따른다. 운영 가격 변경과 신규 상품 공개는 후속 출시 작업이다.
- App Store에서 신규 말풍선·투척물을 구매하면 서버 승인 뒤 해당 종류에 즉시 장착한다. `Transaction.currentEntitlements`, 시작 재검증과 사용자가 요청한 복원은 소유권만 동기화하고 현재 장착을 덮지 않는다. Windows는 지원이 선언된 기존 24개 상품을 렌더링·미리보기·보유 장착하지만 구매는 지원하지 않는다.
...
- 공용 유료 상품 33종은 `assets/v1/commerce-catalog.json`을 기준으로 한다. 유료 캐릭터 12종·말풍선 3종·투척물 18종이며, 독립 애착 물건 12종은 모두 1,100원이다. 현재 판매 ID 33개와 과거 복원 ID 10개로 Apple 검증 ID는 43개다. 신규 단품 구매에는 애착 물건이 포함되지 않는다. 이전 포함 상품과 전환 전 기존 지급의 물건 권리는 출처별로 보존한다. Windows는 이번 신규 9개를 제외한 기존 24개 상품을 유지한다.
...
- 일반 창의 최소화·최대화·닫기 버튼은 실제 앱 테마를 따라 밝은/어두운 배경 모두에서 식별 가능하게 표시하며, 테마 변경 시 갱신한다. 창 축소·확대 중에도 레이아웃을 안정적으로 갱신한다.
- 프로필의 닉네임·설명과 입력창은 충분한 폭에서는 좌우로, 좁은 폭에서는 닉네임·설명·입력창 순서로 세로 배치한다. 캐릭터 선택 타일은 폭 128~144 DIP·최대 5열을 유지한다. 말풍선·투척물은 최대 4열로 간격을 제외한 가용 폭 전체를 균등 분할하며 144 DIP 상한을 적용하지 않는다. 좁은 화면에서는 2열을 기본으로 가용 폭에 맞춰 열 수를 조정하며 스켈레톤도 종류별 열 수·폭 규칙을 따른다. 마지막 행도 위 행과 같은 열 폭을 유지하며 이름은 줄바꿈한다. 세 종류 모두 선택 여부와 관계없이 포인터를 올리면 둥근 셀 안을 같은 은은한 테마 배경으로 강조한다. 강조 배경과 선택 테두리·체크가 놓이는 셀의 경계는 일치한다. 항목과 선택 표시는 잘리지 않아야 하며, 레이아웃 전환은 Windows 애니메이션 설정이 켜져 있을 때만 항목을 동시에 부드럽게 이동한다. 프로필 재진입 시 기존 글자와 이미지는 중첩된 등장·페이드·순차 지연 없이 즉시 함께 표시한다.
- 상점 캐릭터 미리보기의 연속 투척 요청은 진행 중인 비행과 피격 표시를 처음부터 다시 시작하지 않는다. 오버레이와 같은 순서로 진행 중인 동작을 마친 뒤 다음 투척을 처리하며, 미리보기 종료·상품 전환·애니메이션 끄기 시 남은 요청을 정리한다.
- 투척 Broadcast의 카탈로그 ID와 렌더링 자산 ID는 모두 번들의 고정 매핑으로 투척물 14종을 표시한다. 미등록·누락 ID는 기본 말랑공으로 표시한다.
...
- 캐릭터 목록 갱신·화면 해제로 생긴 빈 선택은 자동 저장하지 않고 마지막 확정 선택을 유지한다. 미등록 캐릭터 ID는 서버 프로필 변경 전에 거부한다. 성공한 닉네임·캐릭터 저장은 프로필·로컬 재시작 캐시·내 그룹 멤버 표시에 함께 반영한다.
...
- Windows 일반 제거는 설치 언어·설치 위치·설치 버전을 담은 머신 단위 설치 키, 제거 등록, `sidey:` protocol 등록과 데스크톱 사용자의 자동 실행 값을 항상 삭제한다. 데스크톱 사용자의 `%LOCALAPPDATA%\SIDEY` 설정·로그와 Credential Manager 로그인 자격 증명은 제거 화면에서 각각 선택한 경우에만 삭제한다.
- Windows Setup과 제거기는 머신 단위 작업만 관리자 권한으로 수행하고, 설치 완료 앱 실행과 `%LOCALAPPDATA%`·Credential Manager·HKCU 정리는 데스크톱 셸 사용자 토큰으로 수행한다. 기존 NSIS 설치는 보호된 same-volume staging과 rollback 폴더를 사용해 파일·머신 등록·공용 바로가기를 한 transaction 경계에서 복구하며 동시 실행을 직렬화한다. 이전 정식 MSI가 감지되면 자동 제거하지 않고 기존 설치를 그대로 둔 채 중단하며, 사용자가 Windows 설정에서 MSI를 제거한 다음 Setup을 다시 실행한다.
...
- 루트 `README.md`는 한국어 해요체로 제품 소개, macOS·Windows 접이식 설치 안내, 문제 신고·기여·감사·라이선스와 정책 링크를 제공한다. 영어·일본어·중국어 간체·번체·러시아어·우크라이나어 번역은 `docs/readme/README.<locale>.md`에 두고 언어 전환·이미지·문서 상대 경로를 함께 관리한다. 버전·빌드·업데이트 내역은 README에 반복하지 않고 각 플랫폼 설치 안내에서 공식 GitHub Releases로 연결한다. App Store는 공개 제품 페이지로 연결하고 Microsoft Store는 공개 제품 주소가 확인될 때까지 준비 중으로 표시한다. 기술 스택·아키텍처·백엔드·개발·빌드 설명은 공개 README에 두지 않는다.
...
유료 캐릭터는 별빛 우파루파만 2,200원이고 나머지는 1,100원이다. macOS는 별빛 우파루파·기니피그·원숭이·친칠라·수달·돼지·나무와 신규 시바견·오리·똥·떡볶이·쿼카를 지원하고, Windows는 신규 5종을 제외한 기존 7종을 지원한다. macOS와 Windows 선택 목록에는 무료 5종과 현재 계정이 활성 소유권을 가진 유료 캐릭터만 표시한다. 환불 또는 소유권 만료 시 로컬 선택을 햄스터로 되돌리며, 다른 사용자의 유료 캐릭터를 렌더링할 때는 보는 사람의 소유권을 요구하지 않는다. 친칠라의 canonical ID는 `pixel_chinchilla`이고 과거 `pixel_koala`는 호환 alias로만 정규화한다.
...
기본 시트는 240×24 RGBA이며 idle 2·walk 4·doze 2·offline 2프레임을 가진다. 물체 던지기 action 시트는 각 캐릭터마다 192×24 RGBA이며 24×24 셀의 throw 4프레임과 hit 4프레임을 가진다. 일반 물체 시트는 192×16 RGBA이며 16×16 셀의 회전 8프레임과 충돌 4프레임을 가진다. 모든 캐릭터의 미장착 기본 투척물은 패치 말랑공이다. 캐릭터별 애착 물건 관계는 상점 미리보기용이며 소유권이나 장착을 자동으로 부여하지 않는다. 투명 배경, sRGB, 8-bit RGBA, hard alpha와 integer nearest-neighbor를 유지하고 안티앨리어싱과 실시간 그림자는 사용하지 않는다. 알 수 없는 캐릭터 ID는 햄스터 action과 패치 말랑공으로 fallback한다.

수달·돼지·나무와 고유 투척물(조개·돼지고기·작은 나무), 별도 투척물(눈송이·야구공·왁뿌볼·두쫀쿠)은 [3차 검토](../assets/candidates/otter-pig-tree/round-3/README.md)의 시각 승인을 받았다. 왁뿌볼 음원은 A로 고정하며 돼지고기 `촵` 2차(0.24초)까지 사용자 승인을 받았다. macOS 직배포판·App Store판 양쪽 캐릭터 목록·고유 투척물·장착 가능 유료 투척물·충돌음에 연결한다. 수달·돼지·나무·눈송이·야구공은 1,100원, 왁뿌볼·두쫀쿠는 2,200원이다. App Store 실제 표시는 Apple 현지화 가격을 따른다. 애착 물건은 별도 판매하며 기존 포함 구매 권리는 보존한다. 나무는 우클릭으로 서버 프로필의 정지·걷기 상태를 저장하고 더블 우클릭으로 던지기를 활성화한다. macOS·Windows는 멤버별 최신 revision을 사용한다. 정지 중 호흡·피격·던지기 동작은 유지하며 수평 반전은 하지 않는다. Sidey-dev Debug 로컬 테스트 방에서도 실제 앱 렌더러·충돌음·나무 정지를 검토할 수 있다. 두 macOS 배포판에 효과음 설정·재생·중지 정책과 기절을 동일하게 적용한다. Windows의 기존 콘텐츠는 유지하며 운영 판매 잠금 해제는 포함하지 않는다.

신규 시바견·오리·똥·떡볶이·쿼카는 [character-five 검토 패키지](reviews/character-five/README.md)와 [최종 승인 목록](reviews/character-five/FINAL_ASSETS.md)을 따른다. 서면 계약과 캐릭터·물건 시트, 설명 10개, 충돌음 승인은 완료했다. 시바견·오리·똥·떡볶이는 character-v2, 쿼카는 character-v3를 사용한다. 테니스공·어묵꼬치 소리는 audio-v2 후보 1, 휴지는 audio-v3 B, 잎사귀는 audio-v3 A다. 기존 삑삑 오리의 승인 시트·음원·상품을 재사용하므로 신규 상품은 캐릭터 5개와 투척물 4개다. 원본 18개 파일의 커밋·경로·SHA-256은 `assets/v1/character-five-source.json`에 기록한다. macOS 양 배포판의 프로필 선택·원격 표시·상점·투척·충돌음에 연결하고 Windows 신규 콘텐츠는 후속 범위로 남긴다. 이번 준비만으로 앱 출시나 운영 판매가 시작되지는 않는다.
...
기절은 macOS 직배포판과 App Store판에 적용한다. 대상별 최근 10초 충돌에서 중복 이벤트를 제외한 10번째 피격에 6초 기절하며 누적을 비운다. 기절 중 추가 충돌은 효과·소리만 재생하고 시간을 연장하지 않는다. 자기 확대·던지기는 차단하되 메시지·타이핑·Presence는 유지한다. 외형 변경은 누적을 유지하고 방 전환·대상 제거·잠금·연결 재수립은 임시 상태를 정리한다. 로컬 충돌 시각을 사용하므로 다른 클라이언트와 기절 시각이 다를 수 있다.
...
모든 신규 상품은 계정 귀속 비소모성 영구 해금이다. 장착은 모든 그룹에 공통이며 구매 승인 직후 해당 종류의 상품을 자동 장착한다. macOS에서 말풍선 장착 해제는 기본 흰 말풍선, 투척물 장착 해제는 공통 말랑공으로 돌아간다. 애착 물건 12종은 독립 상품이며 과거 포함 구매자의 권리는 지급 출처별로 보존한다. Windows도 모든 캐릭터의 미장착·알 수 없는 투척물은 기본 말랑공으로 표시한다.

| 종류 | ID | 표시명 | 가격(부가세 포함) |
| --- | --- | --- | ---: |
| 말풍선 | `bubble_bunny_pink` | 핑크 토끼 말풍선 | 2,200원 |
| 말풍선 | `bubble_butter_chick` | 버터 병아리 말풍선 | 2,200원 |
| 말풍선 | `bubble_starry_cat` | 별밤 고양이 말풍선 | 2,200원 |
| 투척물 | `throwable_bouncy_heart` | 통통 하트 | 1,100원 |
| 투척물 | `throwable_toy_cannon` | 미니 대포 | 3,300원 |
| 투척물 | `throwable_squeaky_duck` | 삑삑 오리 | 1,100원 |
...
- direct판의 `내 프로필`은 캐릭터 저장 영역 아래에 `말풍선`과 `투척물` 선택 영역을 항상 표시한다. 각 영역은 보유 상품이 없어도 장착 상태인 `기본 말풍선` 또는 `기본 말랑공` 타일을 먼저 배치하고, 현재 snapshot의 활성 entitlement가 있는 상품만 catalog 순서대로 4열에 추가한다. 환불·회수·미보유 상품은 표시하지 않는다.
...
- App Store 배포 타깃은 `.appStore` availability를 사용하고 캐릭터 12종·말풍선 3종·투척물 18종을 제공한다. StoreKit에서 받은 상품명·설명·현지화 가격과 구매·pending·복원·보유·오류 상태를 표시하고 외부 결제 안내는 표시하지 않는다. 구매 성공 transaction은 서버 승인 전까지 finish하지 않으며 시작·foreground에서 `currentEntitlements`, 사용자가 누른 복원에서만 `AppStore.sync()`를 사용한다. 신규 cosmetic 구매 경로만 서버 승인 후 장착 RPC를 호출하며 복원과 자동 재검증은 기존 선택을 유지한다.
...
- Windows `내 프로필`은 캐릭터 아래에 말풍선·투척물 선택 영역을 항상 표시한다. 각 영역은 `기본 말풍선` 또는 `기본 말랑공`을 먼저 보여주고 snapshot의 활성 entitlement가 있는 상품만 추가한다. 캐릭터·말풍선·투척물 선택은 각각 별도 서버 action으로 즉시 적용하고 서버 성공 응답만 반영하며 진행 중인 같은 종류의 중복 요청을 막는다. 닉네임은 정규화한 현재 값과 draft가 다를 때만 `닉네임 변경하기` 버튼을 입력란 가까이에 표시하고 유효한 2~8자 값만 명시적으로 저장한다.
...
- Windows는 공용 `assets/v1/commerce-catalog.json`의 33개 중 Windows 지원 24개 상품을 검토된 원본 커밋에서 생성해 앱에 묶어 사용해요. 수달·돼지·나무와 눈송이·야구공·왁구공·두쫀쿠, 애장품 7종을 같은 렌더러와 프로필 장착 경로에 추가해요. 애장품 entitlement는 캐릭터 소유권과 독립적이며, 캐릭터 구매만으로 애장품을 보유한다고 판정하지 않아요. 미장착 기본값은 모든 캐릭터의 패치 말랑공이에요. 서버의 활성 entitlement가 회수되면 기존 snapshot 처리로 기본값에 복귀해요.
...
공개 상점은 기본 제공 7종(캐릭터 5·말랑공·기본 말풍선)과 공통 catalog의 유료 33종(캐릭터 12·투척물 18·말풍선 3)을 한국어·영어·일본어로 소개한다. 가격과 캐릭터–애착 물건 연결은 `assets/v1/commerce-catalog.json`에서 읽고 캐릭터 카드·상세에서 애착 물건의 별도 판매와 가격을 표시한다. 상세의 애착 물건 카드를 누르면 해당 투척물 미리보기로 전환한다. 승인 충돌음은 사용자가 미리보기 안의 소리 버튼을 켰을 때 충돌 시점에 재생하고 닫기·상품 전환·백그라운드 진입에서 중단한다. 두쫀쿠의 충돌 4프레임은 0.58초 동안 보여준다. 나무 설명에는 macOS 앱의 우클릭 정지·재개 기능을 안내한다. 가격은 macOS 직배포 기준이며 App Store 가격은 앱에서 확인하도록 안내한다. 이 소개는 결제 활성화나 Windows 신규 콘텐츠 지원을 보장하지 않는다.
...
- 던지기 렌더링: 캐릭터 throw/hit과 투척물 sprite 선택 분리, 모든 캐릭터의 기본 말랑공 fallback, 신규 일반 투척물, 대포의 전경 몸통 emitter·심지탄·피격자 몸통 폭발, 4개 화면 가장자리, 이동 목표 추적, 다중 피격 재시작, 대상 상태 전환, 방 전환 중 stale 이벤트를 검증한다. 12명이 0.5초마다 던지는 초당 24개 부하에서 활성 투사체 32개 이하, p95 frame time 40ms 이하, 100ms 이상 UI hang과 지속 메모리·handle 증가가 없어야 한다.
...
투척물 미리보기 19종은 앱과 동일한 충돌음을 제공한다. 소리 토글은 기본 꺼짐이고, 켜면 즉시 한 번 재생한 뒤 애니메이션 충돌 시점에 재생한다. 모션 감소 설정에서는 토글을 켤 때만 재생하며, 소리를 끄거나 상세를 닫거나 다른 상품을 열거나 페이지가 숨겨지면 기존 재생을 중지한다. 버튼은 SVG 스피커·음소거 아이콘과 켜짐 색을 사용한다.

상점·checkout·결과 화면은 `commerce-catalog.json`에서 생성한 동일한 33종 정의를 사용한다. checkout 표시는 서버가 확인한 주문명·가격을 유지하며 이미지 경로는 배포 base를 보존한다. 기본 제공 7종은 유료 상품 차집합이 아닌 명시 목록이고, 영어·일본어 유료 상품 번역은 한 파일에서 관리하며 누락 시 빌드에 실패한다. 서버의 판매 잠금·가격 재검증·권리 지급 조건은 유지한다. 상품 생성·검사 절차는 `assets/README.md`를 따른다.
...
2026-09-15 승인 콘텐츠 공용 연결: 승인 패키지에서 선택된 캐릭터 기본·동작 시트 10개, 투척 시트 4개와 충돌음 4개를 바이트 변경 없이 공용 원본과 웹 미리보기에 연결한다. 오리는 기존 삑삑 오리를 재사용한다. 공용 카탈로그는 유료 33개이며 웹은 한·영·일 이름·설명과 별도 물건 관계를 같은 원본에서 제공한다. `assets/v1/character-five-source.json`은 승인 출처와 파일별 SHA-256을 기록한다. 신규 자산의 macOS 지원 표시는 해당 앱 번들의 통합·검증 뒤 별도 공용 변경으로 선언하고 Windows에는 아직 선언하지 않는다. 웹은 사용 중인 앱에서 지원 상품과 App Store 가격을 확인하도록 안내한다.
