...
- Windows 인터넷 연결 프로필의 가용성 변경을 OS 이벤트로 감시한다. 인터넷 경로가 사라지면 회색 연결 대기를 표시하고 소켓 재접속·진행 중 복구 조회를 취소하며 인터넷 복구 이벤트 전에는 재접속하지 않는다. 로컬·가상 어댑터만 활성인 상태를 인터넷 연결로 취급하지 않는다. 서버만 응답하지 않는 경우에는 최대 30초 간격의 자동 복구를 유지하며, OS의 제한된 인터넷 연결은 연결 시도를 허용한다.

- 실시간 연결 시작 시 독립 채널 구독을 병렬로 요청하고 성공 응답 뒤에만 연결 완료로 전환한다. 소켓 단절 시 응답 대기를 즉시 해제하며, 첫 재접속은 350ms 후, 연속 실패는 8·16·30초 대기로 제한한다. 초기 구독 실패에도 활동 감시 등 나머지 초기화를 계속한다. 그룹 조회와 구독은 별도 단계이므로 조회 완료 시각을 연결 완료 시각으로 간주하지 않는다.
