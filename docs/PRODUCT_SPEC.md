...
- 문서 버전: 0.8
- 최종 갱신: 2026-09-09
- 상태: macOS `v1.1.0`(build 24) 정식 공개·production 상점 판매 잠금, Windows 네이티브 `v1.1.1` 정식 출시
...
- 설정의 소리 카드는 기본 ON인 캐릭터 효과음 스위치와 0–100% 음량 슬라이더를 제공한다. 음원 선택·별도 미리듣기 버튼은 두지 않고 상점에서 실제 투척 충돌로 소리를 확인한다. 사용자가 음량을 변경한 뒤 슬라이더에서 포인터를 놓을 때만 기본 말랑공 피격음을 최종 음량으로 한 번 재생한다. 드래그 중·조절 도중 잠시 멈춘 상태·설정 저장 완료에서는 재생하지 않는다. 키보드 조절은 키를 놓을 때 재생하며, 중복 완료 이벤트는 다시 재생하지 않는다. 초기 로드·상태 동기화·저장 실패 복구에서는 소리를 내지 않고 0%·설정 화면 이탈·창 닫기에는 피드백을 정리한다. 슬라이더 0% 변경은 토글 OFF, 양수 변경은 ON으로 연결하며 OFF에서도 슬라이더 조절은 가능하다. 토글을 직접 끌 때는 음량을 유지하고, 0%에서 켜면 이번 실행의 마지막 양수 음량 또는 기본 100%로 복원한다. 조용히 모드와 독립적으로 동작한다. 음량은 SIDEY 효과음에만 적용하고 Windows 전체 음량은 변경하지 않는다. Windows preferences schema 4의 `characterSoundEffectsEnabled`는 누락 시 true, `characterSoundEffectsVolume`은 누락 시 100이다. 저장된 false와 0%를 유지하고 범위를 벗어난 음량은 0–100으로 제한하며, 로드 시 0%이면 토글도 OFF로 정규화한다. 토글과 음량은 함께 저장한다. 드래그 중 음량은 즉시 반영하고 설정 저장은 잠시 묶어서 처리하며, 저장 실패는 이전 상태로 복구한다.
...
- Windows 애니메이션 설정을 실행 중 감시한다. OFF이면 상점은 정지 장면과 완성된 말풍선 예시를 표시하고 반복 타이머·수동 확대/투척·미리보기 소리를 정리한다. ON 복귀는 첫 장면부터 시작한다. 기절은 고정 수면 프레임·별·링, 장착 진행은 정적 표식·텍스트로 표시한다. 일반 오버레이 산책의 기존 정책은 유지한다.
...
- Windows 기본 CTA는 현재 정식 v1.1.1의 고정 Setup EXE를 직접 가리키며 저장소에서부터 활성 링크로 제공한다. Pages Actions는 버전에 맞는 정식 Release의 단일 Windows 설치 자산과 고정 URL을 확인한 경우에만 배포를 계속한다.
...
Windows 영어·일본어는 단어별 직역보다 현지 사용자에게 자연스러운 앱 문구와 동물 이름, 일상 대화를 사용하되 기능·제한·결제 의미를 바꾸지 않는다. 날짜·시간은 선택한 앱 언어가 아닌 컴퓨터의 지역 형식(날짜 순서, 구분자, 12·24시간 표기 등)과 시간대 설정을 따른다. 숫자는 선택한 언어의 표준 형식을 사용한다. 상품 가격은 언어와 무관하게 실제 결제 통화인 원화로 표시하며 환율을 추정해 바꾸지 않는다.
