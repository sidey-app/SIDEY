...
- 온보딩 완료·오버레이 표시 설정·저장된 닉네임이 있으면 시작 직후 저장된 내 캐릭터를 회색 오프라인 상태로 표시한다. 인증 복원·그룹 조회·메시지 이력 수신을 기다리지 않는다. 서버 스냅샷 수신 시 실제 그룹 멤버로 교체하고 Realtime 연결 전에는 온라인으로 보이지 않게 한다. 초기 연결 실패에도 로컬 표시를 유지하며, 저장된 숨김 설정과 온보딩 흐름은 존중한다. 로컬 시작 화면은 프로필·그룹·권한을 생성하지 않고 투척·메시지 이벤트를 보내지 않는다.
