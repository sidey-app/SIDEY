...
- 투척물별 피격음 8종과 공통 확대음 1종을 사용한다. 시각 효과와 동일한 투척물 ID 해석·fallback을 사용하며 실제 충돌·확대 시작에서 한 번 재생한다. 발사·비행·축소·기절·회복음은 추가하지 않는다.
- 설정의 `표시` 다음에 `소리 / 캐릭터 효과음` 스위치를 둔다. 설명은 `현재 그룹에서 캐릭터가 맞거나 커질 때 효과음을 재생합니다.`다. 신규·기존 사용자 모두 ON, 로컬 저장·즉시 적용·OFF 시 현재 재생 중단으로 정한다. 조용히 모드와 독립적이며 직배포판에만 설정 schema 9와 `characterSoundEffectsEnabled`를 추가한다. App Store판 schema 8은 유지한다.
...
- 실제 충돌·확대 이벤트와 기절 판정은 장면이 소유하고 오디오 서비스는 재생·자원·출력 장치 변경을 소유한다. API·Broadcast payload·Presence는 변경하지 않는다. `APP_STORE` 컴파일 조건과 직배포 전용 리소스 소속으로 신규 코드·설정·WAV·별 PNG를 분리한다.
