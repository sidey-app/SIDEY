...
- 설정의 소리 카드는 기본 ON인 캐릭터 효과음 스위치와 8종 선택·미리듣기를 제공한다. OFF일 때 미리듣기는 비활성화하고 조용히 모드와 독립적으로 동작한다. Windows preferences schema 4의 `characterSoundEffectsEnabled`는 누락 시 true이며 저장된 false를 유지한다. 저장 실패는 이전 상태로 복구한다.
