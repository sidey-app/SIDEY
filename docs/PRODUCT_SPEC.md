...
- Windows 최근 기록의 40 DIP 캐릭터 슬롯은 시스템 accent 10% 배경과 12 DIP 모서리를 사용한다. 기록의 현재 사용자 `나` 표식은 accent 글자와 accent 12% 배경을, 그룹 설정의 `나` 표식은 secondary 글자와 quaternary 계열 중립 배경을 사용하며 라이트·다크 테마에 맞춰 갱신한다.
