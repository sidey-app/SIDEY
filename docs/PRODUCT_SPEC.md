...
- 승인 WAV 8개를 Windows `Assets/Impacts/<투척물 ID>/<투척물 ID>.wav`에 원본 그대로 보관하고 설치 루트의 같은 상대 경로 한 곳에 배포한다. 오버레이와 상점의 실제 충돌에서 재생하며 최대 4개 동시 재생·80ms 시작 간격·500ms 초과 요청 폐기를 적용한다. 설정 OFF·잠금·절전·출력 장치 변경·종료 시 소리를 정리한다. 오류는 해당 재생만 생략한다.
...
Windows UI 문구는 `ko-KR`·`en-US`·`ja-JP`·`zh-CN`·`zh-TW`·`uk-UA`·`ru-RU` 카탈로그를 사용하며 처음에는 시스템 UI 언어를 감지한다. 중국어 싱가포르·Hans는 간체로, 홍콩·마카오·Hant는 번체로 대응한다. 설정 페이지 맨 아래에는 언어 설명 왼쪽과 한국어·English·日本語·简体中文·繁體中文·Українська·Русский 드롭다운 오른쪽의 단일 행을 두며 별도 언어 섹션 제목과 설명은 추가하지 않는다. 비영어권 언어의 행 제목에는 `/ Language` 보조 표기를 유지한다. 선택한 언어를 로컬 설정에 저장한 뒤 재실행 없이 문구만 갱신하며 창·오버레이·연결과 작성 중인 초안, 현재 선택을 유지한다. 저장 실패 시 기존 선택과 언어를 유지한다. 저장된 선택이 없으면 `SIDEY_LANGUAGE`로 로컬 검증 언어를 지정할 수 있고 다른 언어는 한국어로 fallback한다. 앱의 미리보기 닉네임과 예시 메시지도 선택한 언어를 사용한다. 앱 시작 전 런처의 오류 안내는 시스템 UI 언어 또는 검증 환경 변수를 따른다. 언어별 키와 포맷 인자 일치를 자동 검사하며, 상점 미리보기에 별도의 더블클릭 조작 안내 문장은 표시하지 않는다. macOS 구현과 Windows 설치 약관의 한국어 원문은 이 변경으로 수정하지 않는다.
