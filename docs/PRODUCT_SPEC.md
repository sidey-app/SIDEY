...
- 승인 WAV 8개를 Windows `Assets/Audio/Impacts/<투척물 ID>.wav`에 원본 그대로 보관하고 설치 루트의 같은 상대 경로 한 곳에 배포한다. 오버레이와 상점의 실제 충돌에서 재생하며 최대 4개 동시 재생·80ms 시작 간격·500ms 초과 요청 폐기를 적용한다. 설정 OFF·잠금·절전·출력 장치 변경·종료 시 소리를 정리한다. 오류는 해당 재생만 생략한다.
