...
- 유효한 메시지를 전송하면 즉시 닫는다. 낙관적 전송 실패 시 원문을 복구하고 다시 열어 포커스한다.
...
- 상태: 온라인·자리 비움 doze·오프라인 curled sleep·재연결·타이핑, 내 캐릭터 항상 표시, 오프라인 숨김, 방 전환 UUID diff
...
- 창: 월드 항상 위·클릭 통과, 내 캐릭터 52×52 hotspot만 포인터 수신, composer 초기 숨김·열기·Esc·전송·실패 복구, 기록 일반 창
