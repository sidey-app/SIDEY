...
- 프로필의 닉네임·설명과 입력창은 충분한 폭에서는 좌우로, 좁은 폭에서는 닉네임·설명·입력창 순서로 세로 배치한다. 캐릭터·말풍선·투척물 선택 타일은 폭 128~144 DIP를 유지하고 좁은 화면에서는 2열, 넓은 화면에서는 최대 5열로 배치한다. 마지막 행은 같은 타일 폭을 유지하며 이름은 줄바꿈한다. 항목과 선택 표시는 잘리지 않아야 하며, 레이아웃 전환은 Windows 애니메이션 설정이 켜져 있을 때만 항목을 동시에 부드럽게 이동한다. 프로필 재진입 시 기존 글자와 이미지는 중첩된 등장·페이드·순차 지연 없이 즉시 함께 표시한다.
