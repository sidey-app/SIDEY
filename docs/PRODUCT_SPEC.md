...
- direct development와 App Store의 캐릭터 상품 카드를 선택하면 폭 600pt·콘텐츠 맞춤 높이의 상세 시트를 열고 상단에 540×280pt SpriteKit 미리보기 무대를 하나만 배치한다. 강제 최소 높이와 빈 공간용 spacer를 두지 않고 무대·상품명·설명·구매 button 사이에 필요한 간격만 유지한다. 상품 타일 자체는 정적 이미지를 유지한다. 무대 하단에는 라이트·다크 appearance에 대응하는 중립색 4px 픽셀 플랫폼을 24pt 높이로 그리며 신규 PNG를 추가하지 않는다.
...
- 캐릭터 상품은 해당 캐릭터 한 명을 초록 상태 점과 닉네임 `모카`로 표시해 걷기와 idle을 반복한다. 말풍선 상품 타일은 테마 배경과 좌상단 캐릭터 장식만 보여주고 본문 text를 표시하지 않는다. 상세 무대에서는 햄스터 `모카`와 고양이 `두부`가 걸으며 `모카 . → .. → ... → 저메추좀 해줘 → 두부 . → .. → ... → 곱도리탕 어때?`를 반복한다. 각 입력 중 상태는 1초, 완성 메시지는 2초 유지하며 해당 테마의 실제 배경·글자·좌상단 캐릭터 장식·꼬리·가장자리 위치 보정을 사용한다. 장식은 좌상단 모서리 바깥에 걸치고 본문 여백이나 말풍선 크기를 늘리지 않는다.
...
- 상점: 760pt 6열과 좁은 폭 5열·4열, 세 탭, 한 줄 타일 상태, 말풍선 타일의 본문 미표시, 상세 sheet의 단일 구매/보유 button, 콘텐츠 맞춤 높이와 불필요한 상하 빈 공간 부재, 상품별 독립 loading/error, 키보드 탐색·focus·VoiceOver, 860×640·1000×760 라이트·다크 렌더를 검증한다. 상세의 540×280 무대, 48pt 캐릭터·4px 플랫폼, 3배 pulse가 잘리지 않아야 하며 Reduce Motion에서 반복 작업이 없는 정적 첫 장면을 확인한다. 시나리오별 멤버·닉네임·초록 상태, 두 말풍선의 실제 타이핑→본문 순서·1초/2초 timing·스타일·본문 크기를 늘리지 않는 좌상단 모서리 장식, 캐릭터·말풍선 이동과 투척물 캐릭터 고정, 0.35초 첫 투척·정확한 1초 간격·좌우 교대·상품 ID, 일반 투척물과 대포의 throw/projectile/impact/hit 순서를 검사한다. 상세 시트를 반복해서 열고 닫은 뒤 반복 작업과 SpriteKit scene이 남거나 메모리가 지속 증가하지 않아야 한다. production은 action 호출과 animation Task·SKView가 0개이고 검정 68%·픽셀 자물쇠·잠금 안내만 접근성에 노출되어야 한다. App Store 타깃은 기존 캐릭터 4종 동적 상세와 전체 빌드를 회귀 검증한다.
