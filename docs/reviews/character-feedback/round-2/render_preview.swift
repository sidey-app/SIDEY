...
// A stationary one-logical-pixel orbit. Star coordinates and timing stay at V1 values.
let ringContext = canvas(23, 11)
var ringPixels = Set<Int>()
for step in 0..<720 {
    let angle = Double(step) / 720 * 2 * Double.pi
    let x = Int(round(11 + 11 * cos(angle)))
    let y = Int(round(5 + 5 * sin(angle)))
    ringPixels.insert(y * 23 + x)
}
func drawRing(_ context: CGContext, origin: CGPoint, front: Bool?) {
    for pixel in ringPixels.sorted() {
        let x = pixel % 23
        let y = pixel / 23
        let isFront = y <= 5
        if let front, isFront != front { continue }
        context.setFillColor(rgb(isFront ? 0xD7B868 : 0xA68C58))
        context.fill(CGRect(x: origin.x + CGFloat(x), y: origin.y + CGFloat(y), width: 1, height: 1))
    }
}
drawRing(ringContext, origin: .zero, front: nil)
let ring = ringContext.makeImage()!
...
png(ring, "ring-v2.png")
png(scaled(ring, by: 8), "ring-v2-8x.png")
png(scene(time: 0, phase: 0, poseOverride: 1), "hamster-stun-frame-8-v2.png")
png(scene(time: 0, phase: 0.6, poseOverride: 2), "hamster-stun-frame-9-v2.png")
...
png(scaled(board.makeImage()!, by: 6), "hamster-stun-review-v2.png")
...
gif("hamster-stun-orbit-v2.gif", duration: 2.4, fixedPhase: true, bg: 0x202532)
gif("hamster-stun-timeline-v2.gif", duration: 7.2, fixedPhase: false, bg: 0xFAF7F1)
...
let manifest: [String: Any] = ["review_round": 2, "approval": "pending", "assets": assets,
    ...
    "ring": ["width_logical_pixels": 1, "size_logical_pixels": [23, 11],
        "origin_logical_pixels": [21, 14], "front_color": "#D7B868", "back_color": "#A68C58"],
    "feedback": "V1 star orbit retained; thin ring added at user's request. Full V2 approval pending.",
