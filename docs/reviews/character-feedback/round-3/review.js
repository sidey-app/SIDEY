(() => {
  ...
  function candidateCard(candidate) {
    ...
    label.append(title);
    if (candidate.selection === 'chosen_round_2') {
    ...
    buttons.append(
      button('한 번 듣기', () => play(candidate.file, `${candidate.label} · 재생 중`), `${candidate.label} 한 번 듣기`),
      button('세 번 듣기', () => play(candidate.repeat_file, `${candidate.label} · ${candidate.repeat_interval_seconds}초 간격 3회`), `${candidate.label} 세 번 듣기`, true),
    );
  ...
  for (const group of groups) {
    ...
    if (group.comparison_file) {
      ...
      headingRow.append(button(`${order} 비교`, () => play(group.comparison_file, `${group.label} · ${order} 순서로 비교`), `${group.label} 순서대로 비교`, true));
    }
    const description = document.createElement('p');
  ...
  document.querySelector('#prepare-feedback').addEventListener('click', () => {
    const selections = groups.map(group => {
      const fixed = sounds.find(item => item.group_id === group.id && item.selection === 'chosen_round_2');
