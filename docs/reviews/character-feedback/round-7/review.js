(() => {
  ...
  function updateCue() {
    ...
    cue.textContent = active ? (cueKind === 'chat' ? '말풍선!' : '피격!') : '대기';
  ...
  function candidateCard(candidate) {
    ...
    buttons.append(
      ...
      button(candidate.kind === 'chat' ? '채팅 3개 듣기' : '피격 3회 듣기', () => play(candidate.repeat_file, `${candidate.label} · ${candidate.repeat_interval_seconds}초 간격 3회`, [0, 1, 2].map(n => .15 + n * candidate.repeat_interval_seconds), candidate.kind), `${candidate.label} 세 번 듣기`, true),
    );
