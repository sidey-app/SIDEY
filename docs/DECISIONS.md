...
| 공개 상점·판매자 정보 | 한국어 랜딩 상단 탐색과 히어로 다음 영역에 별빛 우파루파 1종의 상점 목록을 공개하고 `store.html` 상품 안내로 연결한다. 랜딩 하단에는 배송일자를 `결제 완료 즉시 사용 가능`으로 표시하고 교환 불가·즉시 제공 뒤 단순 변심 철회 제한·법정 사유 전액 환불을 안내한다. 이어 상호 `싸이디(SIDEY)`, 대표자 `류태현`, 사업자등록번호 `388-53-01259`, 주소 `경기도 용인시 기흥구 서천동로21번길 20-6`, 연락처 `010-9270-2973`·`ryu200112@gmail.com`, 통신판매업 `신고 면제(간이과세자)`를 작은 글씨로 게시한다 | 고정 상품·정책 URL은 유지하고 한국어 랜딩에서 상품과 판매 조건을 바로 확인할 수 있게 한다. 사업자등록증의 생년월일·QR·동호수는 저장소와 웹사이트에 넣지 않는다. 통신판매업 신고 면제 표시는 현재 간이과세자 지위를 전제로 하며 과세유형 변경 시 재검토한다. |
