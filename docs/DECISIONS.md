...
| 이동 | 선택한 가장자리와 평행한 1차원 track에서 로컬 랜덤 산책, 간헐적 idle, 부드러운 상호 회피 | 발 기준선과 법선 좌표는 고정하고 접선 좌표만 바꾼다. 위치는 서버나 Realtime으로 전송하지 않으며 혼잡 시 겹침을 허용한다. |
...
| 입력창 | 기본 숨김인 400×56 입력창을 내 캐릭터 클릭 또는 메뉴바 `메시지 작성`으로 열고, 유효한 전송 뒤 마지막 전송 시점부터 10초간 유지한 뒤 자동으로 닫기 | 연속 메시지는 창을 다시 열지 않고 보낼 수 있게 하며 전송할 때마다 타이머를 갱신한다. Esc·재클릭은 즉시 닫고, 전송 실패로 원문을 복구할 때는 예약 닫힘을 취소한다. 내 캐릭터 위 52×52 NSPanel만 클릭을 받고 월드는 계속 클릭 통과한다. |
...
- 상태 전환, 오프라인 숨김, 방 전환 UUID diff, 최대 8자 닉네임과 상태 점 간격, 말풍선 교체·eviction·만료·이동 중 추적을 검증한다.
