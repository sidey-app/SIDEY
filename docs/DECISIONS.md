...
| 서버 계약 | 운영에 적용한 `20260831030000_expand_room_capacity_and_reduce_message_retention.sql`이 방당 12명과 메시지 7일 보관을 강제하고 사용자당 5개 방 제한은 유지 | 이미 적용된 core migration을 수정하지 않고 후속 migration으로 계약을 변경해 로컬 초기화와 운영 DB가 같은 최종 schema에 도달한다. 임시 20명 staging 계약은 승격하지 않는다. |
...
| 배포 채널 | 현재 공개본은 버전 `0.2.0`, 빌드 `10`의 `v0.2.0` GitHub 정식 release다. 다음 배포는 버전 `1.0.0`, 빌드 `11`의 `v1.0.0` 정식 release로 준비한다 | `1.0.0`은 사용자가 선택한 첫 메이저 버전 표기이며 Developer ID 서명·Hardened Runtime·Apple 공증을 다시 통과해야 공개한다. 고정 `Zzz`, 방 정원 12명, 메시지 7일 보관과 기존 안정화 작업을 모두 포함한다. 12명 30분 장시간 계측은 정식판의 지속 검증 항목으로 유지한다. |
