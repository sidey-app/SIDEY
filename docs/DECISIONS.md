...
| macOS 상점 카드 | `꾸미기·상점`은 2열 상품 그리드를 사용하되 외부 섹션 카드와 내부 미리보기 카드 배경을 제거해 상품 카드 한 단계만 보이게 한다. 카드에는 최대 210pt 영역의 96pt 캐릭터, 좌상단 상태 배지, 우상단 접근 가능한 `hand.tap` 아이콘 미리보기, 이름·가격 한 행, 설명과 상태별 구매 버튼을 둔다. `1회 구매`, 텍스트형 미리보기, 카드별 `부가세 포함`, 중복 `보유 중` 버튼은 표시하지 않고 세금·Google 연결·소유권·연령·환불 안내는 그리드 아래 한 번만 표시한다 | 상품 상태·결제 action은 기존 `AppModel`·`SettingsActions`에 유지하고 반복되는 `StoreProductCard`는 카드별 미리보기 animation과 취소 가능한 Task만 소유한다. 정보 중복과 세 겹 카드 구조를 줄이면서 구매 상태와 접근성은 유지한다. |
| macOS 결제 | Google identity가 연결된 기존 SIDEY 계정만 시스템 브라우저의 토스페이먼츠 결제를 시작하고 서버 주문·승인·소유권 확인 뒤에만 `보유 중`으로 바꾼다. 주문 token은 일회용이며 서버 가격과 결제 승인 결과를 원본으로 사용한다 | 클라이언트 금액·success URL을 신뢰하거나 결제사 비밀키를 앱에 넣지 않는다. Windows 구매 UI는 별도 결정 전까지 추가하지 않는다. |
| 유료 캐릭터 소유권 | 첫 상품은 `pixel_starlight_upalupa` 별빛 우파루파다. macOS 선택 목록은 무료 5종과 현재 계정의 활성 소유권만 보여주며 환불되면 햄스터로 복귀한다. 친구의 유료 캐릭터를 보는 데 시청자의 소유권은 필요하지 않는다 | 미구매 상품을 선택 화면에 광고처럼 노출하지 않고 서버 소유권을 선택 가능 여부의 원본으로 사용한다. |
...
| 서버 계약 | 운영에 적용한 기존 migration은 수정하지 않는다. `20260901000000_security_hardening.sql`을 후속 적용해 epoch 기반 Realtime, private invite HMAC, 서버 event·message rate limit, 안전한 익명 계정 정리와 묶음 retention 알림을 강제한다 | forward-only migration으로 로컬 초기화와 운영 DB가 같은 최종 schema에 도달하게 한다. migration과 보안 대응 macOS 클라이언트는 함께 배포하며, 구버전 클라이언트의 기존 topic 계약은 의도적으로 폐기한다. |
