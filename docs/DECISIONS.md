...
| 입력창 | 기본 숨김인 400×56 입력창을 내 캐릭터 클릭 또는 메뉴바 `메시지 작성`으로 열기 | 내 캐릭터 위 52×52 전용 NSPanel만 클릭을 받고 월드는 계속 클릭 통과한다. Esc·재클릭·전송은 초안을 규칙에 맞게 보존하거나 닫고, 440×76 회피 구역은 열렸을 때만 활성화한다. |
...
| 배포 채널 | 버전 `0.2.0`, 빌드 `3`의 Apple Silicon 앱을 `v0.2.0-alpha.1` GitHub pre-release로 배포 | Developer ID 서명·공증과 장시간 수동 검증 전에는 정식 stable로 오인시키지 않는다. |
...
- 상태 전환, 오프라인 숨김, 방 전환 UUID diff, 말풍선 교체·eviction·만료·이동 중 추적을 검증한다.
