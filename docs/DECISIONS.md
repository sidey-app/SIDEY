...
| 현재 구현 범위 | macOS production에는 유료 캐릭터 4종의 잠긴 상점을 배포하고, 격리된 `Sidey-dev`에서만 staging PortOne 테스트 결제를 허용한다. Windows 네이티브 `1.0.3` 정식판은 별도로 제공한다. | production 판매 개시와 결제 검증을 분리한다. Windows의 무료 5종 선택·공유 renderer·구매 미지원 정책은 변경하지 않는다. |
...
| macOS Realtime 상태 분리 | 실제 DB·ephemeral 채널 구독을 뜻하는 `transportConnected`와 실제 단절 뒤 snapshot·메시지 보정 완료를 뜻하는 `recoveryReconciled`를 분리한다. 프로필·닉네임·방 이름 갱신은 연결 상태와 원격 Presence 캐시를 유지하고 metadata snapshot만 다시 읽으며, 방 UUID 또는 `realtime_epoch`가 달라지거나 실제 transport가 끊긴 경우에만 재구독·회색 재연결 상태를 사용한다. | 일반 `structure_changed`를 단절로 오인해 본인이 회색으로 고착되거나 친구 상태가 빨강·초록으로 흔들리는 문제를 막는다. 프로필 저장은 `upsert_profile` 반환값을 즉시 적용하고 Realtime 채널 재설정·Presence 재게시를 하지 않는다. |
...
| 청약철회·환불 | 구매 승인 후 7일 이내에는 사용 여부와 관계없이 PortOne 전액 취소·재조회를 완료한 뒤 구매 entitlement를 회수하고 사용 중인 해당 프로필만 `pixel_hamster`로 변경한다. 그 이후에도 법정 사유가 확인되면 전액 환불한다. | complimentary 지급은 환불 대상이 아니다. 취소 함수는 운영 인증과 멱등키를 요구한다. |
| 유료 캐릭터 소유권 | 우파루파는 1,900원, 기니피그·원숭이·친칠라는 각 990원이다. macOS 선택 목록은 무료 5종과 현재 계정의 활성 소유권만 보여주며 production 상점 잠금 중에도 이미 가진 캐릭터는 선택·렌더링할 수 있다. | 미구매 상품을 선택 화면에 광고처럼 노출하지 않고 서버 소유권을 선택 가능 여부의 원본으로 사용한다. Windows 코드는 변경하지 않는다. |
...
| 공개 상점·판매자 정보 | 한국어 랜딩 본문에는 상품을 두지 않고 상단 `상점` 탭만 별도 `store.html`로 연결한다. 상점 페이지는 우파루파·기니피그·원숭이·친칠라 4종과 각 가격, macOS 앱 내 구매 경로, 구매·환불 조건을 고정형 사용자 문구로 안내하며 출시 준비·배포 환경·테스트 채널 같은 내부 상태는 노출하지 않는다. 랜딩 하단의 판매자·배송·환불 정보는 유지한다. checkout은 공개 구매 링크로 노출하지 않고 staging/dev 주문 token으로만 접근한다. | 랜딩과 상점의 책임을 분리하고 공개 상품 안내가 내부 개발 상태에 따라 계속 바뀌지 않게 한다. production 판매 잠금 중 결제 진입점은 공개하지 않으며 사업자등록증의 생년월일·QR·동호수는 저장소와 웹사이트에 넣지 않는다. |
...
| 서버 계약 | 운영에 적용한 기존 migration과 Toss 거래는 감사 이력으로 수정하지 않는다. 신규 forward-only `20260902050000_paid_characters_portone_v2.sql`이 판매 잠금·pending 취소·4종 가격·complimentary 지급·PortOne 상태 검증·4종 프로필 소유권 검사를 추가한다. | 직접 Toss 실행 함수 권한·Edge Function·시크릿은 제거하고 주문 가격·유료 프로필 선택·환불 회수는 서버가 강제한다. staging은 같은 migration 뒤에만 `sales_enabled=true`, `payment_environment=test`로 명시 전환한다. |
...
| 배포 채널 | 현재 공개본은 버전 `1.0.4`, 빌드 `15`의 `v1.0.4` GitHub 정식 stable release다 | production 상점은 잠금 상태로 내고 PortOne 판매는 열지 않는다. 프로필 변경은 Realtime transport와 Presence 캐시를 재설정하지 않는다. Windows `1.0.3` 배포물은 이번 macOS 릴리스에서 변경하지 않는다. |
