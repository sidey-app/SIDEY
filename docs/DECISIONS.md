...
| 공개 상점·판매자 정보 | 한국어 랜딩 본문에는 상품을 두지 않고 상단 `상점` 탭만 별도 `store.html`로 연결한다. 상점 페이지는 우파루파·기니피그·원숭이·친칠라 4종, 각 가격과 출시 예정 상태를 표시한다. 랜딩 하단의 판매자·배송·환불 정보는 유지한다. checkout은 공개 구매 링크로 노출하지 않고 staging/dev 주문 token으로만 접근한다. | 랜딩과 상점의 책임을 분리하고 production 판매 잠금 중 결제 진입점을 공개하지 않는다. 사업자등록증의 생년월일·QR·동호수는 저장소와 웹사이트에 넣지 않는다. |
