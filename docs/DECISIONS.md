...
| 상태 표현 | 온라인=`산책/idle + 초록`, 자리 비움=`선 채 졸기 + 주황 점 + 14pt 주황 Zzz·약 2pt 어두운 외곽선·0.55↔1 alpha·3pt 부유`, 오프라인=`웅크린 잠 + 빨강 + 저채도·75% alpha`, 재연결=`정지 + 회색` | 자리 비움과 오프라인을 포즈·채도·보조 효과까지 다르게 표시한다. |
...
| macOS 설정 화면 | 각 설정 섹션은 아이콘·제목·설명을 카드 위에 두고 카드 안의 항목은 제목 아래 설명과 오른쪽 토글·선택 컨트롤로 정렬한다. 상세 영역은 가로 44pt·세로 40pt 바깥 여백과 카드 안 24pt 여백을 사용하며, 그룹 목록·새 그룹 만들기·초대 코드 참여는 각각 독립 카드로 구분 | 설정 효과를 조작 전에 이해할 수 있게 하고, 서로 다른 서버 mutation인 그룹 생성과 참여를 한 입력 묶음처럼 보이지 않게 한다. 상태와 action 소유권은 기존 `AppModel`·`SettingsActions`에 유지한다. |
...
| macOS Keychain 고지 | 로그인 세션이나 그룹 초대 코드를 읽기 위해 macOS Keychain 인증 UI가 필요한 경우 `SIDEY는 로그인 세션과 그룹 초대 코드를 안전하게 불러오기 위해 macOS 키체인 접근이 필요합니다.`라는 현지화된 이유를 시스템 인증창에 표시 | ad-hoc에서 Developer ID 서명으로 전환할 때처럼 macOS가 기존 항목 접근을 다시 확인해도 사용자가 요청 목적을 판단할 수 있게 한다. 별도 비밀번호를 수집하거나 Keychain 밖으로 비밀값을 옮기지 않는다. |
...
| 배포 채널 | 버전 `0.2.0`, 빌드 `7`의 Developer ID 서명·Hardened Runtime·Apple 공증 Apple Silicon 앱을 `v0.2.0-alpha.5` GitHub pre-release로 배포 | 설명이 있는 Liquid Glass 설정 카드, 분리된 그룹 생성·참여 흐름, 설정 내 Sparkle 업데이트 확인, Keychain 접근 목적 고지를 묶은 alpha 패치다. 30분 장시간 기준 전에는 stable로 표시하지 않는다. |
