...
| 공개 상점·판매자 정보 | 한국어 랜딩 본문에는 상품을 두지 않고 상단 `상점` 탭만 별도 `store.html`로 연결한다. 상점 페이지는 macOS 앱과 같은 300~360px adaptive 정사각형 캐릭터 카드 그리드를 사용하며 현재 별빛 우파루파 1종을 표시하고 이후 상품을 같은 그리드에 추가한다. 랜딩 하단에는 배송일자를 `결제 완료 즉시 사용 가능`으로 표시하고 교환 불가·즉시 제공 뒤 단순 변심 철회 제한·법정 사유 전액 환불을 안내한다. 이어 상호 `싸이디(SIDEY)`, 대표자 `류태현`, 사업자등록번호 `388-53-01259`, 주소 `경기도 용인시 기흥구 서천동로21번길 20-6`, 연락처 `010-9270-2973`·`ryu200112@gmail.com`, 통신판매업 `신고 면제(간이과세자)`를 작은 글씨로 게시한다 | 랜딩과 상점의 책임을 분리하고 상품 수가 늘어날 때 상점 카드만 반복 추가할 수 있게 한다. 사업자등록증의 생년월일·QR·동호수는 저장소와 웹사이트에 넣지 않는다. 통신판매업 신고 면제 표시는 현재 간이과세자 지위를 전제로 하며 과세유형 변경 시 재검토한다. |
...
| Windows 설치·업데이트 | `1.0.3` 정식판은 머신 단위 major upgrade·repair와 downgrade 차단을 지원하고, Pages의 Windows 전용 manifest를 시작 시 한 번과 트레이·설정의 수동 확인에서 사용한다. 일반 제거는 설치된 `Uninstall.exe`를 통해 앱 파일과 현재 사용자의 `%LOCALAPPDATA%\SIDEY` 설정·로그 및 Credential Manager의 `SIDEY/` 로그인 정보를 함께 삭제한다 | 기존 per-user·Burn 테스트 설치는 설치 context와 등록 방식이 달라 사용자가 먼저 제거한다. 설치 폴더·시작 메뉴에서 `Uninstall.exe`를 직접 실행하거나 Windows 설정·MSI에서 제거해도 같은 정리 경로를 사용하며, 데이터 정리는 upgrade·repair에는 실행하지 않는다. 업데이트는 `windows-v<version>`의 고정 MSI URL과 manifest 내부 SHA-256이 유효할 때만 사용자 승인을 받아 다운로드·검증한 뒤 설치기를 실행하며 무인 자동 설치는 하지 않는다. Release에 별도 `.sha256` 파일은 두지 않는다. |
