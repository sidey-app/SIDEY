...
def main():
    ...
    if len(sys.argv) > 2 and sys.argv[2] in ('merge', 'squash', 'commit'):
        return
    if any(Path(git('rev-parse', '--git-path', state)).exists()
           for state in ('MERGE_HEAD', 'CHERRY_PICK_HEAD', 'REBASE_HEAD', 'rebase-merge', 'rebase-apply')):
        return
    author = re.search(r'<([^>]+)>', git('var', 'GIT_AUTHOR_IDENT'))
    committer = re.search(r'<([^>]+)>', git('var', 'GIT_COMMITTER_IDENT'))
    if not author or not committer or author[1].lower() != committer[1].lower():
        return
    ...
    if not meaningful.strip():
        return  # Never turn an empty/aborted message into a trailer-only commit.
    trailers = git('interpret-trailers', '--parse', str(message))
    if any(re.match(r'co-authored-by\s*:', line, re.I)
           and re.search(r'<codex@openai\.com>\s*$', line, re.I)
           for line in trailers.splitlines()):
        return
    subprocess.run(['git', 'interpret-trailers', '--in-place', '--if-exists', 'addIfDifferent',
                    '--trailer', 'Co-authored-by: codex <codex@openai.com>', str(message)], check=True)
